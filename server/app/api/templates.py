...
def _auth_scope(auth: AuthContext) -> str:
    if auth.mode == "user" and auth.church:
        safe = re.sub(r"[^\w가-힣\-]", "_", auth.church)
        return f"church_{safe}"
    return "guest"
...
def list_templates(auth: AuthContext = Depends(get_optional_auth_context)):
    scope = _auth_scope(auth)
    try:
        items = list_templates_for_scope(scope)
    except Exception as e:
        raise HTTPException(500, detail=f"템플릿 목록 조회 실패: {e}")
    return {"templates": items, "count": len(items), "scope": scope}
...
def get_default(auth: AuthContext = Depends(get_optional_auth_context)):
    scope = _auth_scope(auth)
    template_id = get_scoped_default_template(scope)
    return {"template_id": template_id, "scope": scope}


@router.put("/api/templates/default")
async def set_default(request: Request, auth: AuthContext = Depends(get_optional_auth_context)):
    try:
    ...
    if template_id:
        try:
            resolve_template_path(template_id, scope=scope)
        except FileNotFoundError:
            raise HTTPException(404, detail=f"템플릿 '{template_id}'을 찾을 수 없습니다.")

    set_scoped_default_template(template_id, scope)
    logger.info("[template/default] 기본 템플릿 설정: %s (scope=%s)", template_id, scope)
    return {"template_id": template_id, "scope": scope}
...
def delete_template(template_id: str, auth: AuthContext = Depends(get_optional_auth_context)):
    scope = _auth_scope(auth)
    try:
        delete_scoped_template(scope, template_id)
    except FileNotFoundError as e:
        raise HTTPException(404, detail=str(e))
    except Exception as e:
        raise HTTPException(500, detail=f"삭제 실패: {e}")
    logger.info("[template/delete] %s 삭제 (scope=%s)", template_id, scope)
...
async def template_preview(template_id: str, auth: AuthContext = Depends(get_optional_auth_context)):
    scope = _auth_scope(auth)
    try:
        template_path = resolve_template_path(template_id, scope=scope)
    except FileNotFoundError:
        raise HTTPException(404, detail=f"템플릿 '{template_id}'을 찾을 수 없습니다.")
...
async def upload_template(
    ...
    try:
        ...
        dest = os.path.join(_scoped_template_dir(scope), safe_name)
        shutil.copy2(tmp_path, dest)
        logger.info("[template/upload] 저장 완료: %s (scope=%s)", safe_name, scope)
